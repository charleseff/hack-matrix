...
def step(
    ...
    is_warp_action = action == (ACTION_PROGRAM_START + 3)  # PROGRAM_WARP = 3
...
def _advance_stage(state: EnvState) -> EnvState:
    ...
    state = _generate_stage_content(state, new_stage)

    return state


def _generate_stage_content(state: EnvState, stage: jnp.int32) -> EnvState:
